...
var CtrlBuilder = function () {
  ...
  _createClass(CtrlBuilder, [{
    ...
    value: function define(method, path, fn) {
      ...
      path = (0, _path.join)('/', path).replace(/\\/g, '/');
      var fullPath = (0, _path.join)('/', self.prefix, path).replace(/\\/g, '/');
      debug('mount route: ' + method.toUpperCase() + ' ' + fullPath);+self.router[method](path, fn);
    }
  }]);
